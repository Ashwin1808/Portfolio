...
const LOOP_LEFT = ["linux", "docker", "k8s", "terraform"];
const LOOP_RIGHT = ["aws", "ci/cd", "prometheus", "grafana"];

/** One ring of the ∞ — the tools travel around it, staying upright. */
function OrbitLoop({ labels, dur }: { labels: string[]; dur: number }) {
      ...
      >
        {labels.map((label, i) => (
          <motion.div
            ...
            transition={{ duration: dur, repeat: Infinity, ease: "linear" }}
          >
            <span className="flex items-center gap-1.5 whitespace-nowrap rounded-full border border-cream/25 bg-[#0d0b09]/85 px-2 py-0.5 font-mono text-[8px] uppercase tracking-[0.16em] text-cream/70 backdrop-blur-sm">
              <span
...
/**
 * 01 — The front of the sheet, in space. Centered manifesto, a DevOps
 * loop (∞) carrying the tools along the bottom edge, a ringed planet,
        ...
        <motion.div style={{ y: reduced ? 0 : contentY }} className="mx-auto flex max-w-[900px] flex-col items-center text-center">
          ...
          >
            ...
            <motion.h1
              ...
              className="mt-10 font-serif text-[clamp(3.4rem,10.5vw,8.75rem)] leading-[0.9] tracking-[-0.02em]"
            ...
            >
              <p className="max-w-[460px] text-[15px] leading-[1.85] text-cream/55">
            ...
            >
              <div className="flex items-center justify-center">
                {reduced ? null : (
                  <>
                    <OrbitLoop labels={LOOP_LEFT} dur={18} />
                    <OrbitLoop labels={LOOP_RIGHT} dur={23} />
                  </>
                )}
              </div>
